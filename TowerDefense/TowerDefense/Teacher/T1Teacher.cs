...
namespace AttackOnGeek
{
    ...
    public class T1Teacher : Teacher
    {
        public T1Teacher(Texture2D texture, Texture2D paperTexture, Vector2 position,Texture2D paperBreakTexture, int totalFrame = 6)
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            for (int i = 0; i < paperList.Count; i++)
            {
                ...
                {
                    if (target.StudentType != 0 && paper.Paperstate == PaperState.FLYING)
                    {
