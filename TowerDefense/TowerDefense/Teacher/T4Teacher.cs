...
{
    public class T4Teacher : Teacher
    {
        public T4Teacher(Texture2D texture, Texture2D paperTexture, Vector2 position, Texture2D paperBreakTexture, int totalFrame = 6)
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (paperTimer >= 0.75f && target != null)
            {
                Paper paper = new Paper(paperTexture, paperBreakTexture, Vector2.Subtract(center,
                    new Vector2(paperTexture.Width / 2)), rotation, 6, damage);
            ...
            for (int i = 0; i < paperList.Count; i++)
            {
                ...
                {
                    if (target.StudentType != 3 && paper.Paperstate == PaperState.FLYING)
                    {
